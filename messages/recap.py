...
def get_month_leaders(standings, current_week):
  ...
  month_name = next((name for name, (start, end) in SCORING_RANGES.items()
                     if current_week in range(start, end + 1)), None)

  # Current month: top 3 medalists
  ...
  top_month_scorers = utils.get_medalists(month_standings, lambda x: x['score'],
                                          2)
  month_leaders.append(utils.get_context_from_arr(top_month_scorers))

  # Previous months: list the single best team for each prior month, most recent first
  month_names = list(SCORING_RANGES.keys())
  try:
  ...
  if idx > 0:
    ...
    for i in range(idx - 1, -1, -1):
      prev_name = month_names[i]
      prev_start, prev_end = SCORING_RANGES[prev_name]
      prev_month_standings = []
      for team in standings:
        prev_score = sum(team.scores[prev_start - 1:prev_end])
        prev_month_standings.append({
            'team_abbrev': team.team_abbrev,
            'score': prev_score
        })

      # pick the top team (best winner) for this previous month
      prev_top = max(prev_month_standings, key=lambda x: x['score'])
      prev_score_str = utils.format_number(
          prev_top['score'],
          target_length=utils.get_number_length(prev_top['score']),
          decimal_places=2)
      prev_lines.append(
          f'{prev_name}: {utils.get_medal_string('first', prev_score_str, prev_top["team_abbrev"])}'
      )

    month_leaders.append(utils.get_context_from_arr(prev_lines))
