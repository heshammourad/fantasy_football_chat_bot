...
td_keys = [
    'defensivePlusSpecialTeamsTouchdowns', 'passingTouchdowns',
    'receivingTouchdowns', 'rushingTouchdowns'
]
teams = {
    team: {
        category: 0
        for category in list(positions.keys()) + ['TD']
    }
    for team in utils.get_teams()
}
trophies = [('High Score', 'fire'), ('Low Score', 'ice_cube'),
            ('Blowout', 'sunglasses'), ('Squeaker', 'sweat_smile'),
            ('Lucky', 'four_leaf_clover'), ('Unlucky', 'rage'),
            ('Overachiever', 'chart_with_upwards_trend'),
            ('Underachiever', 'chart_with_downwards_trend'),
            ('Best Manager', 'white_check_mark'), ('Worst Manager', 'x'),
            ('Stud', '+1'), ('Dud', '-1'), ('Benchwarmer', 'dotted_fire')]
...
def get_leaderboards(league):
    ...
    for i, (category, data) in enumerate(positions.items()):
        if data['top'] == 0:
            continue
        medalists = utils.get_medalists([{
            'team_abbrev': team,
            **stats
        } for team, stats in teams.items()],
                                        lambda x: x[category],
                                        decimal_places=data['decimals'])
        leaderboards_section.append(
            utils.get_mrkdwn_from_arr(f':{data["emoji"]}: *{category}s*'))
...
def process(team_abbrev, lineup):
    ...
    roster_breakdown = {
        pos: []
        for pos in positions.keys() if positions[pos]['top'] > 0
    }
...
def get_trophies(league):
    ...
    for box_score in box_scores:
        ...
        performances.extend([(away_team, away_performance),
                             (home_team, home_performance)])

        players.extend([(away_team, p, away_score - home_score)
                        for p in get_active_players(box_score.away_lineup)])
        players.extend([(home_team, p, home_score - away_score)
                        for p in get_active_players(box_score.home_lineup)])

        benchwarmers.extend([
            (away_team, combo)
            for combo in get_benchwarmers(box_score.away_lineup)
        ])
        benchwarmers.extend([
            (home_team, combo)
            for combo in get_benchwarmers(box_score.home_lineup)
        ])
    ...
    trophies_values.append(
        f'{utils.get_team(blowout[0])} blew out {utils.get_team(blowout[1])} by *{utils.format_number(blowout[2] - blowout[3], decimal_places=2)}* points ({utils.format_number(blowout[2], decimal_places=2)}-{utils.format_number(blowout[3], decimal_places=2)})'
    )
    trophies_values.append(
        f'{utils.get_team(squeaker[1])} lost to {utils.get_team(squeaker[0])} by *{utils.format_number(squeaker[2] - squeaker[3], decimal_places=2)}* points ({utils.format_number(squeaker[2], decimal_places=2)}-{utils.format_number(squeaker[3], decimal_places=2)})'
    )

    # Lucky and Unlucky
    points = sorted([
        score for sublist in [result[2:4] for result in results]
        for score in sublist
    ])
    sorted_lucky = sorted(results, key=lambda x: x[2])
    lucky_rank = utils.get_rank_ordinal(points,
                                        sorted_lucky[0][2],
                                        reverse=True)
    trophies_values.append(
        f'{utils.get_team(sorted_lucky[0][0])} finished with the {lucky_rank} highest score, but still came away with the win'
    )

    sorted_unlucky = sorted(results, key=lambda x: x[3], reverse=True)
    unlucky_rank = utils.get_rank_ordinal(points, sorted_unlucky[0][3])
    trophies_values.append(
        f'{utils.get_team(sorted_unlucky[0][1])} had the {unlucky_rank} highest score, but still ended up taking the L'
    )
    ...
    sorted_performances = sorted(performances,
                                 key=lambda x: x[1],
                                 reverse=True)
    overachiever, underachiever = sorted_performances[0], sorted_performances[
        -1]
    trophies_values.append(
        f'{utils.get_team(overachiever[0])} were over their projection by *{utils.format_number(abs(overachiever[1]), decimal_places=2)}* points'
    )
    trophies_values.append(
        f'{utils.get_team(underachiever[0])} were under their projection by *{utils.format_number(abs(underachiever[1]), decimal_places=2)}* points'
    )

    # Best and Worst Managers
    accuracy = list(
        functionality.optimal_team_scores(league, full_report=True).items())
    worst = accuracy[-1]
    best = []
    last_pct = None
    for value in accuracy:
        ...
        last_pct = pct
        best.append(
            f'{utils.get_team(value[0].team_abbrev)} scored *{pct}%* of their optimal score'
        )
    trophies_values.append('\n'.join(best))
    trophies_values.append(
        f'{utils.get_team(worst[0].team_abbrev)} left {utils.format_number(worst[1][2], decimal_places=2)} points on the bench, scoring only *{utils.format_number(worst[1][3], decimal_places=1)}%* of their optimal score'
    )

    # Studs and Duds
    player_achievement = sorted(
        [p for p in players],
        key=lambda x: x[1].points - x[1].projected_points,
        reverse=True)
    studs = search_players(
        player_achievement,
        condition=lambda x: x[2] > 0 and x[1].points - x[
            1].projected_points > x[2],
        sort=lambda x: x[1].points - x[1].projected_points - x[2],
        reverse=True)
    stud = studs[0] if studs else player_achievement[0]
    stud_text = f'*{stud[1].name}* - {utils.get_team(stud[0])} outperformed projection by *{ utils.format_number(stud[1].points - stud[1].projected_points, decimal_places=2)}*'
    if studs:
    ...
    dud_text = f'*{dud[1].name}* - {utils.get_team(dud[0])} underperformed projection by *{  utils.format_number(dud[1].projected_points - dud[1].points, decimal_places=2)}*'
    if duds:
        dud_text += f' in a {utils.format_number(abs(dud[2]), decimal_places=2)} loss'
    trophies_values.append(dud_text)

    # Benchwarmer
    sorted_benchwarmers = sorted(benchwarmers,
                                 key=lambda x: x[1][1].points - x[1][0].points,
                                 reverse=True)
    benchwarmer_team, (starter, benchwarmer) = sorted_benchwarmers[0]
    trophies_values.append(
        f'*{benchwarmer.name}* - {utils.get_team(benchwarmer_team)} scored *{utils.format_number(benchwarmer.points, decimal_places=positions[benchwarmer.position]["decimals"])}* points on the bench while {     starter.name} scored just {utils.format_number(starter.points, decimal_places=positions[starter.position]["decimals"])}'
    )
...
def get_benchwarmers(lineup):
    ...
    for player in get_bench_players(lineup):
        possible_positions = set([
            p.position for p in active_players
            if p.lineupSlot in player.eligibleSlots
        ])
        possible_replacements = [
            p for p in active_players if p.position in possible_positions
        ]
        benchwarmers.extend([(starter, player)
                             for starter in possible_replacements])
    return benchwarmers
