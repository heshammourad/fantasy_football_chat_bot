...
def get_fields(fields):
  ...
  for i in range(0, len(fields), 2):
    ...
    fields_block.append({'type': 'section', 'fields': fields_section})
  return fields_block
...
def get_mrkdwn_from_arr(arr):
  ...
  return {'type': 'section', 'text': get_mrkdwn_text(text)}


def get_context_from_arr(arr):
  text = '\n'.join(arr) if type(arr) is list else arr
  return {"type": "context", "elements": [get_mrkdwn_text(text)]}


def get_formatted_scoreboard(away_team,
                             away_score,
                             home_team,
                             home_score,
                             is_final=False):
...
def get_rank_ordinal(arr, value, reverse=True):
  ...
  def condition(n, value):
    return n > value if reverse else n < value

  return num2words(sum(1 for n in arr if condition(n, value)) + 1,
                   to='ordinal_num')
...
def get_medalists(teams, field_getter, decimal_places=0):
  ...
  for i, team in enumerate(sorted_teams):
    ...
    try:
      medal = prev_medal if i != 0 and value == field_getter(
          sorted_teams[i - 1]) else medals[i]
    except IndexError:
      break
    prev_medal = medal
